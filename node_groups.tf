...
module "eks_managed_node_group" {
  ...
  subnet_ids = try(each.value.subnet_ids, var.eks_managed_node_group_defaults.subnet_ids, local.subnet_ids)
  ...
  vpc_id                            = try(each.value.vpc_id, var.eks_managed_node_group_defaults.vpc_id, local.vpc_id)
...
module "self_managed_node_group" {
  ...
  max_size                  = try(each.value.max_size, var.self_managed_node_group_defaults.max_size, 0)
  desired_size              = try(each.value.desired_size, var.self_managed_node_group_defaults.desired_size, 0)
  ...
  vpc_id                            = try(each.value.vpc_id, var.self_managed_node_group_defaults.vpc_id, local.vpc_id)
